...
lat_higher = 85
delta = 5
magneticfield="present_B_"
direction = "vert_"

lon_arr = np.array([   -180.00,-177.50,-175.00,-172.50,-170.00,-167.50,-165.00,-162.50,  
...
nlat = len(lat_array_str)
nlon = len(lon_array_str)
...
for lat_idx,lat in enumerate(lat_array_str):
    print(lat)
    filename = "../dat/2x2.5_grid_new/"+file_prefix +'lat_'+ lat + '.txt'
    try:
...
with open("../dat/"+file_prefix+"_rigidity_final.csv","w+") as my_csv:
    csvWriter = csv.writer(my_csv,delimiter=',')
    csvWriter.writerows(cutoff[2,:,:])

with open("../dat/"+file_prefix+"_penumbra_info_final.csv","w+") as my_csv:
    csvWriter = csv.writer(my_csv,delimiter=',')
